...
function createEchart() {
    ...
    newLine.forEach(function (p1, p2, p3) {
        var l;
        if (p1.parentNode.copyrightType == 'WRW') {  //作品关系
            l = {
                source: p1.node.name,
        ...
        }else if(p1.parentNode.copyrightType == 'ORO'){ //版权关系
            l = {
                source: p1.node.name,
                target: p1.parentNode.name,
                value: p1.parentNode.tag,
    ...
    for(var i=0;i<dataList.length;i++){
        ...
        if(dataList[i].workSeqNo){
            dataList[0] = dataList[i];
            dataList[i] = tempNode;
            break;
        }
    }
    var newArryData=dataList;
    for(var i=0;i<dataList.length;i++){
        if(dataList[i].relevantPeople && dataList[i].relevantPeople.length>0){
            for(var j=0;j<dataList[i].relevantPeople.length;j++){
                var newObj={
                    name:dataList[i].relevantPeople[j].peopleNames[0],
                    workType:'',
                    id:dataList[i].id+'_'+j,
                    obbligeType:dataList[i].relevantPeople[j].obbligeType,
                    parents:[{
                        parentNmae:dataList[i].name,
                        parentId:dataList[i].id,
                        tag:dataList[i].relevantPeople[j].relevantType,
                        lineColor:5,
                        copyrightType:'WRW'
                    }],
                    isInitNode:dataList[i].relevantPeople[j].isInitNode,
                    nodeType:'OBBLIGE',
                    labes:[]
                }
                for(var k=0;k<dataList[i].relevantPeople[j].peopleNames.length;k++){
                    if(k>0){
                        newObj.labes.push(dataList[i].relevantPeople[j].peopleNames[k])
                    }
                }
                newArryData.push(newObj)
            }
        }
    }

    relation.setData(newArryData);
    relation.redraw();

}
var offset_X,offset_Y;
var obj = $('#bql-map');
var relation = new RGraph.Relation({
    id: 'bql-map',
    data:{}
}).draw();
obj.click(function(e){
    ...
    relation.click(pageX, pageY);
    var s=relation.click(pageX, pageY);
    console.log(s)
    ...
    // }
    if(s&&s.menu&&s.menu.indexOf('TZ')){
        if(!isLogin()){
            $('.modle-login').show();
            showLogin();
            return;
        };
        var menu=s.menu.split(',');
        $('.ul_nav').find('dl').each(function () {
            $(this).hide();
            if($.inArray($(this).data('type'), menu)>-1){
                $(this).show();
        ...
        $('.now_name').val(s.name).attr('num',s.nodeType);
        $('.module-bql').show();
        $('#menuuu').hide();
    }else if(s.menu&&s.menu.indexOf('TZ')>-1){
        window.open("./"+s.menu.substring(3)+"?from="+$('.fromType').val())
    }
});
...
obj.dblclick(function(e){
    pageX =	e.pageX - this.offsetLeft;
    pageY = e.pageY - this.offsetTop;
    relation.dbclick(pageX, pageY);
});

obj.mousemove(function(e){
    pageX =	e.pageX - this.offsetLeft;
    pageY = e.pageY - this.offsetTop;
    relation.mousemove(pageX, pageY);
    var str = "mousedown 鼠标位置:"+pageX+"-"+pageY+",releativeX:"+relation.relativeX+",relativeY:"+relation.relativeY;
});
...
obj.mousedown(function(e){
        ...
        pageY = e.pageY - this.offsetTop;
        var str = "mousedown 鼠标位置:"+pageX+"-"+pageY+",releativeX:"+relation.relativeX+",relativeY:"+relation.relativeY;
        relation.mousedown(pageX, pageY);
    }
);
